...
const AdminTimeSheet = ({ Id, record, index }) => {
  ...
  useEffect(() => {
    const fetchtimsheetdurationbydate = async () => {
      try {
        setLoading(true);
        const response = await fetch(
    ...
    <div className="bg-white dark:bg-neutral-950 dark:text-white p-2 rounded-lg shadow-lg mb-14 flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2">
        <div className="relative w-full">
          <div className="w-full p-2 border border-gray-300 rounded-lg bg-white dark:bg-neutral-900 dark:border-neutral-700  shadow-lg">
            <div className="flex items-center justify-betwee gap-2 mb-4">
              <button
              ...
              <button
                onClick={handleToday}
                className="p-2 mt-1 bg-sky-100 rounded-lg dark:bg-neutral-800 dark:border-neutral-700 flex items-center gap-2"
              >
                Today
              </button>
            </div>
            <div className="grid grid-cols-7 gap-1 mb-2 bg-sky-50 dark:bg-neutral-950 p-2 rounded-md">
              ...
              {Array.from({ length: daysInMonth }).map((_, day) => {
                // Construct the date in UTC to avoid timezone mismatch
                const date = new Date(
                  Date.UTC(currentYear, currentMonth, day + 1)
                );
                const apiDateKey = date.toISOString().split("T")[0]; // Format: yyyy-MM-dd
                const isRedBg = datesFromApi.has(date.toDateString());
                const durations = timesheetData[apiDateKey]?.reduce(
                ...
                return (
                  <div
                    ...
                    className={classNames(
                      "p-2 cursor-pointer rounded-md h-28 border dark:border-neutral-700",
                      {
